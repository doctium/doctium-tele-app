...
export default function VideoCallScreen() {
  const { appointmentId, topicId, patientName } = useLocalSearchParams<{
      ...
      try {
        ...
        if (consent.status === "NOT_REQUESTED") {
          Alert.alert(
            ...
            "Recording is optional. It will only be enabled if both you and the patient consent.",
            [
        ...
        } else if (consent.status === "PENDING" && !consent.doctorConsented) {
          Alert.alert(
            "Patient requested recording",
    ...
    status === "preview"
      ...
      : status === "error"
        ...
        : status === "connecting"
          ...
          : remoteJoined
            ? mmss
            : `Waiting for ${patientName ?? "patient"}…`;
    ...
    <View style={styles.root}>
      ...
      {!showRemoteVideo ? (
        <View style={StyleSheet.absoluteFill}>
          ...
          <View style={[styles.remote, { paddingTop: insets.top + 60 }]}>
            <Avatar size={132} name={patientName ?? "Patient"} ring />
      ...
      >
        {ZEGO_AVAILABLE && !videoOff ? (
          ...
          >
            <Ionicons name="medkit" size={26} color="rgba(255,255,255,0.85)" />
          </LinearGradient>
        )}
      </Animated.View>
