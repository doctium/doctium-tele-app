...
export const callApi = {
  /** Fetches a Zego token for the current doctor to join a video room. */
  ...
  placeRecordingDisputeHold: async (
    appointmentId: string,
    requestId: string,
    data: { disputeHoldUntil?: string; disputeHoldReason?: string },
  ): Promise<RecordingRequest> => {
    const res = await apiClient.patch(`/appointments/${appointmentId}/recording/requests/${requestId}/dispute-hold`, data);
    return (res as unknown as { data: RecordingRequest }).data;
  },
};
