...
interface Doctor { id: string; name: string; image?: string; designation?: string; }
...
export default function DoctorHolidayPage() {
  ...
  const load = () => {
    ...
    ]).then(([h, d]) => { setHolidays(h.data ?? []); setDoctors(d.data ?? []); }).catch(() => {}).finally(() => setLoading(false));
  };
  useEffect(() => { load(); }, []);
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    try { await apiClient.post('/admin/doctor-holidays', form); toast.success('Holiday added'); setShowAdd(false); setForm({ doctorId: '', date: '' }); load(); }
    catch {}
  ...
  const cols: Column<Holiday>[] = [
    ...
    { key: 'date', header: 'Holiday Date', render: r => <span className="font-semibold">{r.date}</span> },
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="card p-0 overflow-hidden">
        <DataTable columns={cols} data={holidays} keyExtractor={r => r.id} loading={loading} emptyMessage="No holidays scheduled" />
      </div>
      <Modal open={showAdd} onClose={() => setShowAdd(false)} title="Add Doctor Holiday">
        <div className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button onClick={handleAdd} disabled={saving} className="btn-primary flex-1">{saving ? 'Saving...' : 'Add Holiday'}</button>
          </div>
        </div>
      </Modal>
    </div>
