...
export default function SuggestedServicesPage() {
  ...
  const load = () => {
    apiClient
      .get("/admin/suggested-services")
      .then((r: unknown) =>
        setServices((r as { data: SuggestedService[] }).data ?? []),
      )
      .catch(() => {})
  ...
  const cols: Column<SuggestedService>[] = [
    ...
    },
    {
      key: "doctor",
      header: "Suggested By",
      render: (r) => <span className="text-sm">{r.doctor?.name ?? "—"}</span>,
    },
    {
      key: "date",
      header: "Date",
      ...
      render: (r) => (
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-1 text-xs bg-green-50 text-green-600 hover:bg-green-100 px-3 py-1.5 rounded-lg font-medium"
          >
            <CheckCircle size={12} /> Approve
          </button>
          <button
            onClick={() => handleReject(r.id)}
            className="flex items-center gap-1 text-xs bg-red-50 text-red-600 hover:bg-red-100 px-3 py-1.5 rounded-lg font-medium"
          >
    ...
    <div className="space-y-5">
      ...
      <div className="card p-0 overflow-hidden">
        <DataTable
          ...
          keyExtractor={(r) => r.id}
          loading={loading}
          emptyMessage="No service suggestions from doctors"
        />
      </div>
    </div>
  );
