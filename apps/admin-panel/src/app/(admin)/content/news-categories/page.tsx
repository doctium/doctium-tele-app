...
export default function NewsCategoriesPage() {
  ...
  const openEdit = (c: MediaCategory) => {
    setForm({
      id: c.id,
      name: c.name,
      slug: c.slug,
  ...
  const handleDelete = async (id: string) => {
    ...
    await apiClient.delete(`/admin/media/news-categories/${id}`);
    ...
    try {
      if (form.id) {
        await apiClient.patch(
          `/admin/media/news-categories/${form.id}`,
          payload,
        );
        toast.success("Category updated");
      } else {
        await apiClient.post("/admin/media/news-categories", payload);
  ...
  const cols: Column<MediaCategory>[] = [
    {
      key: "name",
      header: "Name",
    ...
    },
    {
      ...
      header: "Description",
      render: (r) => (
        <span className="text-sm text-gray-500 truncate max-w-[320px] block">
          {r.description || "—"}
        </span>
      ),
    },
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="page-title">News Categories</h1>
        <button
          onClick={openAdd}
      ...
      <p className="text-sm text-gray-500 -mt-2">
        Used to group press releases and news posts. The slug is generated from
