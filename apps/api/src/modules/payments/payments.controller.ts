...
export class PaymentsController {
  ...
  getWallet(@CurrentUser() user: JwtPayload) { return this.paymentsService.getUserWallet(user.sub); }

  @Post('wallet/topup')
  @UseGuards(RolesGuard) @Roles('user')
  topUp(@CurrentUser() user: JwtPayload, @Body('amount') amount: number) {
    return this.paymentsService.topUpWallet(user.sub, amount);
  }
  ...
  getDoctorWallet(@CurrentUser() user: JwtPayload) { return this.paymentsService.getDoctorWallet(user.sub); }

  @Get('doctor/wallet/stats')
  @UseGuards(RolesGuard) @Roles('doctor')
  getDoctorEarningsStats(@CurrentUser() user: JwtPayload) { return this.paymentsService.getDoctorEarningsStats(user.sub); }

  @Get('doctor/withdraw-methods')
  @UseGuards(RolesGuard) @Roles('doctor')
  getWithdrawMethods(@CurrentUser() user: JwtPayload) { return this.paymentsService.getDoctorWithdrawMethods(user.sub); }

  @Post('doctor/withdraw-methods')
  @UseGuards(RolesGuard) @Roles('doctor')
  saveWithdrawMethod(@CurrentUser() user: JwtPayload, @Body() dto: Record<string, unknown>) {
    return this.paymentsService.saveDoctorWithdrawMethod(user.sub, dto);
  }

  @Post('doctor/withdraw')
  @UseGuards(RolesGuard) @Roles('doctor')
  requestWithdrawal(@CurrentUser() user: JwtPayload, @Body() dto: { amount: number; withdrawMethodId?: string; bankDetails?: { accountNumber?: string; bankCode?: string; bankName?: string; accountName?: string } }) {
    return this.paymentsService.requestWithdrawal(user.sub, dto);
  }

  @Get('doctor/withdraw-requests')
  @UseGuards(RolesGuard) @Roles('doctor')
  getRequests(@CurrentUser() user: JwtPayload) { return this.paymentsService.getWithdrawRequests(user.sub); }
