...
export class AppointmentsController {
  ...
  @Roles('user')
  book(@CurrentUser() user: JwtPayload, @Body() dto: Record<string, unknown>) {
  ...
  @UseGuards(RolesGuard)
  @Roles('user')
  getUserAppointments(@CurrentUser() user: JwtPayload, @Query('status') status?: AppointmentStatus) {
  ...
  @UseGuards(RolesGuard)
  @Roles('doctor')
  getDoctorAppointments(@CurrentUser() user: JwtPayload, @Query('status') status?: AppointmentStatus) {
  ...
  getRecordingConsent(@Param('id') id: string, @CurrentUser() user: JwtPayload) {
  ...
  respondRecordingConsent(
    ...
    return this.appointmentsService.respondRecordingConsent(id, user, consent === true, {
      ip: req.ip,
      userAgent: String(req.headers['user-agent'] ?? ''),
    });
  }
  ...
  @UseGuards(RolesGuard)
  @Roles('user', 'doctor', 'admin')
  listRecordingAssets(@Param('id') id: string, @CurrentUser() user: JwtPayload) {
  ...
  @UseGuards(RolesGuard)
  @Roles('user', 'doctor', 'admin')
  listRecordingRequests(@Param('id') id: string, @CurrentUser() user: JwtPayload) {
  ...
  setRecordingRequestDisputeHold(
    ...
    return this.appointmentsService.setRecordingRequestDisputeHold(id, requestId, user, body);
  }
  ...
  cancel(
    ...
    const role = user.role === 'user' ? 'PATIENT' : user.role === 'doctor' ? 'DOCTOR' : 'ADMIN';
    return this.appointmentsService.cancelAppointment(id, role as never, reason);
  }
